...
const String PHONE = "+917483404200";
...
const int vibrationPin = 36; // Digital pin connected to the vibration sensor
const int ledpin = 23;
const int disconnectPin = 22; // Digital pin connected to D22
...
void loop() {
   ...
   if (digitalRead(vibrationPin) == LOW) {
    ...
    // You can add additional actions or code here
  }
  else {
    digitalWrite(ledpin, LOW);
  }

  if (digitalRead(disconnectPin) == HIGH) {
...
void sendAlertSMS() {
  ...
  if (newData) {
    ...
    String alertMessage = "Alert: Someone is trying to disturb your jewelry!\n";
    alertMessage += "Location: http://maps.google.com/maps?q=loc:" + String(gps.location.lat(), 6) + "," + String(gps.location.lng(), 6);
    ...
    Serial.println("Failed to get GPS location for alert.");
  }
}